  ...
  return (
    <div className="max-w-md flex flex-col gap-6">
      ...
      <section className="flex flex-col gap-2">
        <h2 className="text-xs font-bold uppercase tracking-wide text-text-secondary">Missing &amp; wanted</h2>
        {wanted.length === 0 ? (
          <p className="text-sm text-text-secondary">Nothing on your wishlist yet.</p>
        ) : (
          wanted.map((item) => (
            <Link
              key={item.id}
      ...
      <section className="flex flex-col gap-2">
        <h2 className="text-xs font-bold uppercase tracking-wide text-text-secondary">Watching for</h2>
        {favorites.length === 0 ? (
          <p className="text-sm text-text-secondary">
            Favorite an author or series from its page to watch for it here.
          </p>
        ) : (
          favorites.map((fav) => (
            <div key={fav.id} className="flex items-center gap-3 rounded-xl border border-border bg-surface px-3 py-2.5">
              <div className="flex-1">
                ...
                <div className="text-xs text-text-secondary capitalize">Favorite {fav.targetType}</div>
              </div>
              <button type="button" onClick={() => removeFavorite(fav.id)} aria-label="Remove favorite">
